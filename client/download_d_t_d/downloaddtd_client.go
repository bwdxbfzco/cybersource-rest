...
// New creates a new download d t d API client with basic auth credentials.
...
// New creates a new download d t d API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
// WithAcceptApplicationXMLDtd sets the Accept header to "application/xml-dtd".
func WithAcceptApplicationXMLDtd(r *runtime.ClientOperation) {
	r.ProducesMediaTypes = []string{"application/xml-dtd"}
}
...
func (a *Client) GetDTDV2(params *GetDTDV2Params, opts ...ClientOption) (*GetDTDV2OK, error) {
	...
	success, ok := result.(*GetDTDV2OK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getDTDV2: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
